...
class FlashItem {
  ...
  final String? word; // 단어 카드 (키릴 한 단어) 또는 plain 문장
  final String? reading; // plain 문장 카드의 한글 독음
  final String tts; // 발음용 텍스트
  final String back; // 뒷면 한국어 뜻
  final String? note; // 뒷면 보조 (포인트/품사)

  const FlashItem(
      {this.tokens, this.word, this.reading, required this.tts, required this.back, this.note});
  ...
  factory FlashItem.fromSpoonfed(SpoonfedSentence s) =>
      FlashItem(word: s.ru, reading: s.reading, tts: s.ru, back: s.ko, note: s.note);
}
